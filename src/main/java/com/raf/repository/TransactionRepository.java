...
List<Transaction> findByPaymentStatus(PaymentStatus paymentStatus, Sort sort);
List<Transaction> findByDeliveryStatus(DeliveryStatus deliveryStatus);
List<Transaction> findByDeliveryStatus(DeliveryStatus deliveryStatus, Sort sort);

List<Transaction> findByBuyerIdAndPaymentStatus(Long buyerId, PaymentStatus paymentStatus);
List<Transaction> findByBuyerIdAndPaymentStatus(Long buyerId, PaymentStatus paymentStatus, Sort sort);
List<Transaction> findBySellerIdAndPaymentStatus(Long sellerId, PaymentStatus paymentStatus);
List<Transaction> findBySellerIdAndPaymentStatus(Long sellerId, PaymentStatus paymentStatus, Sort sort);
...
@Query("SELECT SUM(t.commission) FROM Transaction t WHERE t.paymentStatus = 'PAID'")
BigDecimal getTotalSystemCommission();

@Query("SELECT SUM(t.commission) FROM Transaction t WHERE t.paymentStatus = 'PAID' AND t.transactionDate >= :startDate")
BigDecimal getTotalCommissionSince(@Param("startDate") LocalDateTime startDate);

@Query("SELECT t FROM Transaction t LEFT JOIN FETCH t.inventory inv LEFT JOIN FETCH inv.cropType LEFT JOIN FETCH t.buyer LEFT JOIN FETCH t.seller WHERE t.id = :id")
Optional<Transaction> findByIdWithInventoryAndRelations(@Param("id") Long id);
}
